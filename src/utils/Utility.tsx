...
export const errorOnDev = (...message: any[]) =>
   consoleOnDev('error', ...message)

export const stringFromDate = function(
   date:
      | string
      | number
      | void
...
export const stringFromDateTime = function(
   date:
      | string
      | number
      | void
...
export const phoneNumberMask = (rawValue: string) => {
   const value = rawValue.split('-').join('')
...
export const checkBisNo = (bisNo: string) => {
   ...
   return (
      10 - ((sum + Math.floor((key[8] * Number(bisNo[8])) / 10)) % 10) ===
      Number(bisNo[9])
   )
}

export const checkTelNumber = (tel: string) => {
